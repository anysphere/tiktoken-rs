...
fn main() {
    ...
    let mut odht = File::create(&Path::new(&out_dir).join("odht_gen.rs")).unwrap();
    writeln!(odht, "pub mod data {{").unwrap();
    let mut encoders = File::create(&Path::new(&out_dir).join("encoders_gen.rs")).unwrap();
    writeln!(encoders, "pub mod data {{").unwrap();
...
fn generate(
    ...
    generate_odht(name, odht, mergeable_ranks);
    generate_encoder(name, encoders, mergeable_ranks);
}
