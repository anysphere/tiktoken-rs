  ...
  let get_rank = {
    ...
    |parts: &Vec<(usize, usize)>, start_idx: usize, skip: usize| {
      if (start_idx + skip + 2) < parts.len() {
        ranks.get(&piece[parts[start_idx].0..parts[start_idx + skip + 2].0]).copied()
      } else {
        None
      }
    }
...
fn hash_current_thread() -> usize {
  ...
  let x =
    unsafe { std::mem::transmute::<std::thread::ThreadId, FakeThreadId>(thread::current().id()).0 };
  u64::from(x) as usize
}
...
impl CoreBPE {
    ...
    // Here is a quick and dirty fix:
    {
      let token_is_all_space = |token| {
        self
          .decoder
          .get(token)
          .map(|token_bytes| token_bytes.iter().rev().all(|&b| [b' ', b'\n', b'\t'].contains(&b)))
          .unwrap_or(false)
      };
  ...
  fn _encode_unstable_native(
    ...
    let mut point =
      self.sorted_token_bytes.partition_point(|x| x.as_slice() < unstable_bytes.as_slice());
    ...
    if unstable_bytes.len() > 1 {
      ...
      {
        let mut reencoded =
          byte_pair_encode(&unstable_bytes[..unstable_bytes.len() - last_decoded.1], &self.encoder);
...
impl CoreBPE {
  pub fn new(
    ...
    let special_regex = {
      let parts = special_tokens_encoder.keys().map(|s| fancy_regex::escape(s)).collect::<Vec<_>>();
    ...
    let special_tokens_decoder: HashMap<usize, Vec<u8>> =
      special_tokens_encoder.iter().map(|(k, v)| (*v, k.as_bytes().to_vec())).collect();
    ...
    Ok(CoreBPE {
      ...
      regex_tls: Arc::from((0..MAX_NUM_THREADS).map(|_| regex.clone()).collect::<Vec<_>>()),
      special_regex_tls: Arc::from(
        (0..MAX_NUM_THREADS).map(|_| special_regex.clone()).collect::<Vec<_>>(),
      ),
      sorted_token_bytes,
    })
  }
