...
impl Default for SpecialTokenHandling {
  fn default() -> Self {
    Self { default: SpecialTokenAction::Forbidden, overrides: vec![] }
  }
}
...
impl Encoding {
  ...
  pub fn new(
    ...
    let max_token_value =
      match mergeable_ranks.values().chain(special_tokens.values()).max().copied() {
        Some(value) => value,
        None => {
          return Err(EncodingError::GenericEncodingError("No token values found".to_string()))
        }
      };
    if let Some(explicit_n_vocab) = explicit_n_vocab {
      if mergeable_ranks.len() + special_tokens.len() != explicit_n_vocab {
        return Err(EncodingError::GenericEncodingError(
          "Mismatch between explicit vocab size and actual vocab size".to_string(),
        ));
      }
      if max_token_value != explicit_n_vocab - 1 {
        return Err(EncodingError::GenericEncodingError(
          "Mismatch between max token value and explicit vocab size".to_string(),
        ));
      }
    }

    let mergeable_ranks_max_key_len =
      mergeable_ranks.keys().map(|bytes| bytes.len()).max().ok_or_else(|| {
        EncodingError::GenericEncodingError("No mergeable ranks found".to_string())
      })?;

    let core_bpe = CoreBPE::new(mergeable_ranks.clone(), special_tokens.clone(), pat_str.clone())
      .map_err(|e| {
      EncodingError::GenericEncodingError(format!("Error creating core BPE: {}", e))
    })?;

    let mut prefixes_of_mergeable_ranks = mergeable_ranks
      .keys()
      .flat_map(|bytes| (1..=bytes.len()).map(|i| roll_hash_slice(&bytes[..i])).collect::<Vec<_>>())
      .collect::<HashSet<_>>();
  ...
  pub fn encode(
    ...
    let special_tokens_to_recognize = match special_token_handling.default {
      SpecialTokenAction::Special => {
        &self.special_tokens.keys().map(|s| s.as_str()).collect::<HashSet<_>>()
  ...
  pub fn encode_with_unstable(
    ...
    Ok(self.core_bpe.encode_with_unstable(text, &special_tokens_to_recognize))
  }
  ...
  pub fn decode_tokens_bytes(&self, tokens: Vec<usize>) -> Result<Vec<Vec<u8>>, usize> {
    tokens.into_iter().map(|token| self.decode_single_token_bytes(token)).collect()
  }
...
fn special_token_regex(tokens: &HashSet<&str>) -> Regex {
  let inner = tokens.iter().map(|token| regex::escape(token)).collect::<Vec<_>>().join("|");
...
mod tests {
  ...
  fn test_encoding_memory_usage(encoding_factory: fn() -> Result<Encoding, EncodingFactoryError>) {
    ...
    println!(
      "Memory used by {}: {} bytes",
      std::any::type_name_of_val(&encoding_factory),
      memory_used
    );
